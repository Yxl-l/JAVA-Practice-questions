 ...
 * 授予角色权限，删除角色权限  拦截器
 */
public class TokenInterceptorOperationalAuthorityFunctionBlocker  implements HandlerInterceptor {
    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (ObjectUtils.isEmpty(token) || map == null) {
            ...
            return false;
        } else if (permissions != null &&  (permissions.contains("修改角色权限") || permissions.contains("超级管理员"))) {
            //用户有对应权限或者超级管理员权限才能放行
            log.info("有【修改角色权限】权限，放行");
            return true;
        } else {
            response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);//报错401
            log.info("用户没有【修改角色权限】权限，禁止访问");
            return false;
        }
